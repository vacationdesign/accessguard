...
- **残課題（手動）**: Stripe billing portal のプラン切替は API から設定できなかった（products が silently drop）。Stripe Dashboard →「設定 → Billing → カスタマーポータル」で「顧客がプランを切り替えられるようにする」を有効化し Starter/Pro/Agency を選択すること。これが無いと Starter→Pro のセルフサービス・アップグレード経路が無い（最初の課金者が出る前に要対応）。
