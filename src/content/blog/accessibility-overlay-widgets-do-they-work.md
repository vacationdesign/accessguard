...
- **Color contrast modification** — switch to a high-contrast color scheme
- **Link highlighting** — visually emphasize all links on the page
- **Animation pausing** — stop moving elements
- **Screen reader optimization** — claimed adjustments to improve screen reader compatibility
- **Cursor enlargement** — make the mouse pointer bigger
- **Dyslexia-friendly fonts** — switch to a typeface designed for readability

Some overlay vendors go further, claiming to use AI to detect and repair accessibility barriers in the DOM in real time — automatically adding missing alt text, fixing heading hierarchies, labeling form fields, and adjusting ARIA attributes, all without changes to your source code.
...
- **Low cost** — overlay subscriptions are typically far cheaper than professional remediation.
- **Instant deployment** — one script tag, no development resources required.
- **Marketing reassurance** — vendors often claim compliance with WCAG 2.1 AA, ADA, Section 508, and other standards.
- **No technical knowledge needed** — business owners can install an overlay without understanding HTML, CSS, or ARIA.
...
This is the most critical limitation. An accessibility overlay runs in the browser after your page has loaded. It can manipulate the DOM — adding attributes, changing styles, injecting elements — but it cannot change your actual HTML, your server-rendered content, your CMS templates, or your component source code. Every "fix" it applies is temporary, fragile, and dependent on the overlay script loading and executing correctly.
...
Genuine accessibility is built into the HTML — semantic elements, proper heading hierarchies, form labels, descriptive alt text, ARIA attributes that reflect real widget behavior. These must exist in the source code, not in a runtime patch.
...
- **Screen reader conflicts** — overlays that inject ARIA roles or modify the accessibility tree can cause screen readers to announce elements incorrectly, skip content, or get stuck in unexpected navigation patterns.
- **Keyboard trap creation** — overlay toolbar panels that capture focus can prevent keyboard users from navigating past the overlay to reach the actual page content.
- **Style override conflicts** — users who apply their own CSS overrides for readability find that overlay style modifications conflict with their personal settings.
- **Performance degradation** — overlay scripts add page weight and processing overhead, which disproportionately affects users on older devices or slower connections.
...
- **1.1.1 Non-text Content (Level A)** — Alt text must accurately describe each image's purpose. An AI-generated description may say "a white bottle on a table" when the meaningful alt text is "500ml organic lavender hand lotion, front label view." Only a human who understands the context can write useful alt text.
- **1.2.1 through 1.2.5 (Level A and AA)** — Audio and video content require captions, audio descriptions, and transcripts. No overlay can generate accurate captions for your video content.
- **1.3.1 Info and Relationships (Level A)** — The semantic structure of your content must be conveyed through proper HTML. An overlay cannot reliably determine the intended document structure of content it has never seen before.
- **2.1.1 Keyboard (Level A)** — All functionality must be operable via keyboard. If your custom components use mouse-only event handlers, an overlay cannot retroactively add correct keyboard interaction patterns for every possible widget.
- **3.3.1 Error Identification (Level A)** and **3.3.3 Error Suggestion (Level AA)** — Form validation errors must be clearly identified and described. An overlay has no knowledge of your form's business logic and cannot generate meaningful error messages.
...
The legal theory is straightforward: an overlay that does not actually make the website accessible does not satisfy the ADA's requirement to provide equal access. Courts evaluate whether a website is accessible in practice — not whether the site owner purchased a product that claims to make it so.

### Serial Plaintiff Firms Target Overlay Users

Some plaintiffs' law firms that specialize in high-volume ADA litigation have specifically identified overlay users as targets. A site with an overlay visibly signals that the owner is aware of accessibility obligations but chose a solution that does not work — undermining any argument that the failures were unintentional. For more context on how these lawsuits work, see our guide on [ADA website lawsuits](/blog/ada-website-lawsuits-how-to-protect-your-business).
...
[AccessGuard's free scanner](/) gives you an immediate baseline of your site's WCAG 2.1 violations with specific element references and remediation guidance for each issue. Run a scan, review the results, and you have a concrete list of what needs to be fixed.

Automated scanning should be supplemented with manual testing — keyboard navigation testing, screen reader testing, and content review — because automated tools can only detect a portion of all WCAG violations. But automated scanning is the right starting point because it identifies the highest-volume issues efficiently.
...
- **Navigation** — Can users reach all pages using keyboard alone? Is the menu operable with assistive technology?
- **Forms** — Can users complete your contact form, checkout process, or sign-up flow without a mouse? Are all fields labeled? Are errors clearly communicated?
- **Content** — Do all images have meaningful alt text? Is your heading structure logical? Does text meet contrast requirements?
- **Interactive components** — Do modals, dropdowns, tabs, and accordions work with keyboard navigation and announce their state to screen readers?
...
- **Design reviews** — Check color contrast, readability, and interactive element sizing before designs reach development.
- **Development standards** — Require semantic HTML, proper ARIA usage, and keyboard operability as acceptance criteria for every component.
- **Content guidelines** — Train content editors to write alt text, maintain heading hierarchies, and structure content accessibly.
- **QA testing** — Add keyboard and screen reader checks to your testing process.
...
[AccessGuard's Pro plan](/#pricing) includes scheduled weekly scans across all your pages, historical trend tracking, regression detection, and team notifications. It is the infrastructure that turns a one-time fix into ongoing compliance.

### Step 5: Document Your Efforts

Maintain records of your accessibility work — scan results, remediation logs, and testing reports. If your business ever faces a legal challenge, documented evidence of sustained good-faith efforts significantly strengthens your position. Publish an accessibility statement on your website that describes your commitment, the standards you target, and how users can report barriers.
...
- **Professional credibility** — Accessibility professionals and disability advocates widely oppose overlays. Associating your agency with these products puts you on the wrong side of an industry consensus that is growing stronger.
- **Client liability** — If your client is sued despite having an overlay, the overlay will not serve as a defense. Your client may justifiably ask why you recommended a product that did not protect them.
- **Missed revenue opportunity** — Genuine accessibility services — auditing, remediation, monitoring — represent ongoing revenue more valuable than an overlay referral fee.
...
2. **Run an accessibility scan.** Use [AccessGuard's free scanner](/) to get an honest assessment of your website's WCAG violations — including the violations that persist despite your overlay.
...
[Start your free AccessGuard scan today](/) and see exactly where your website stands — no overlay required.
